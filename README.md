<h1 align="center">Hola! Soy Mica </h1>
<h3 align="center">Estudiante avanzada de Programación (UTN) apasionada por el desarrollo de software y la tecnología.</h3>

<p align="center">
  <img src="https://img.shields.io/badge/UTN_Mar_del_Plata-Estudiante_Avanzada-blue?style=for-the-badge&logo=university&logoColor=white" />
  <img src="https://img.shields.io/badge/Enfoque-Full_Stack_|_Data-success?style=for-the-badge&logo=codefactor&logoColor=white" />
</p>

---

## Sobre mí

Soy estudiante de programación enfocada en el ciclo completo de desarrollo de software (Backend y Frontend). Me motiva resolver problemas lógicos, construir aplicaciones eficientes desde cero y aplicar buenas prácticas de código. Busco integrarme en equipos dinámicos y proyectos reales de alto impacto donde pueda seguir potenciando mis habilidades técnicas.

---

## Formación Actual

* **Tecnicatura Universitaria en Programación** – *UTN Mar del Plata* (Último año en curso)

---

## Lenguajes y Herramientas

<p align="center">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="45" alt="Java" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="45" alt="TypeScript" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" width="45" alt="C" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" alt="HTML5" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45" alt="CSS3" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" width="45" alt="Spring Boot" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angularjs/angularjs-original.svg" width="45" alt="Angular" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original-wordmark.svg" width="55" alt="MySQL" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" alt="Git" />&nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jira/jira-original.svg" width="45" alt="Jira" />
</p>

---

## Áreas de Interés

* **Desarrollo Full Stack:** Diseño de interfaces dinámicas y construcción de arquitecturas robustas.
* **Análisis de Datos & Machine Learning:** Exploración de datos y modelos predictivos para la toma de decisiones.
* **Tecnología Aplicada:** Creación de soluciones innovadoras orientadas a sectores específicos como la salud y la gestión farmacéutica.

---

## Conectemos

<p align="center">
  <a href="https://www.linkedin.com/in/micaela-morano-92631b333?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>
